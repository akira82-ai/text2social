...
(function() {
  ...
  async function init() {
    ...
    try {
      // 首先获取最新的模板列表
      await fetchAvailableTemplates().catch(error => {
        console.warn('⚠️ Could not fetch available templates, using defaults:', error);
      });
  ...
  function createPreviewWindow() {
    ...
    templates.forEach(template => {
      ...
      const displayName = getTemplateDisplayName(template); // 获取模板显示名称
